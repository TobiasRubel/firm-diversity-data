...
def get_pat_paragraphs(data_path):
    df = pd.read_csv(data_path,names=['ids', 'text'], header=None)
    ids = [x for x in df['ids'].values]
    id_and_paragraphs = df.values
    return ids, id_and_paragraphs
...
def preprocess_str_hp_chopped(s, approved_d):
    s_tokens = nltk.word_tokenize(s)
    tags = nltk.pos_tag(s_tokens)
    nouns = [word for word,pos in tags 
             if (pos == 'NN' or pos == 'NNP' or pos == 'NNS' or pos == 'NNPS')]
    s = ' '.join(nouns).lower()
    s = preprocess(s)
    s = [word for word in s.split() if word in approved_d]
    s = ' '.join(s)
    return s
